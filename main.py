...
while is_running:
    if is_name_input:
        ...
        if game_state == "menu":
            ...
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_1:  # Iniciar juego
                        game_state = "playing"
                    elif event.key == pygame.K_2:  # Ver scoreboard
                        game_state = "scoreboard"
                    elif event.key == pygame.K_3:  # Créditos
                        game_state = "credits"
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_RIGHT:  # If right arrow key is pressed
                        player_x_change += 0.7
                    if event.key == pygame.K_r:  # Si se presiona la tecla 'R'
                        is_returning = True
                        game_state = "menu"
                    if event.key == pygame.K_4:  # Volver al menú
                        game_state = "menu"
            ...
            screen.fill((0, 0, 0))  # Limpiar pantalla
            credits_text = credits_font.render("Created by Uriel, Santiago & Edu (4 to back menu) ", True, (0, 255, 255))
